import { logger, RabbitMQClient, EcomEvent } from "@ecom/shared";
...
export async function setupRabbitMQSubscriptions() {
  try {
    await rabbitClient.subscribe(
      "users-service.account-registered",
    ...
    logger.info("[RabbitMQ] Users subscriptions successfully initialized");
  } catch (error) {
